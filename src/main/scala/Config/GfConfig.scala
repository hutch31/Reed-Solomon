...
)
{
  ...
  var chienRootsPerCycle = 0
  var chienErrBitPosLatencyFull = 0
  var chienLatencyFull = 0
  ...
  do {
    ...
    chienLatencyFull = chienErrBitPosLatencyFull
  } while(MSG_DURATION_CORE <= chienLatencyFull)

  //chienRootsPerCycle = 64
  chienErrBitPosLatencyFull = math.ceil((SYMB_NUM-1)/chienRootsPerCycle.toDouble).toInt-1+1 // -1 since it's started same cycle when errLocIf_valid = 1
  ...
  val XlIfToEvLatencyAxisClk = (math.ceil((forneyErrataLocLatencyFull+forneyErrEvalLatencyFull+forneyEvFullLatency) * CLOCK_RATION)).toInt
  ...
  val FdToEvLatencyAxisClk = (math.ceil((forneyErrataLocLatencyFull+forneyErrEvalLatencyFull+forneyEvFullLatency-forneyFdFullLatency) * CLOCK_RATION)).toInt
  ...
  if(XlInvIfToEEXlInvFifoEn)
    println(s"XlInvIf FIFO to EEXlInv block enabled in forney block")
