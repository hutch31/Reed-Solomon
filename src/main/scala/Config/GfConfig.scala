...
)
{
  ...
  var forneyEEXlInvTermsPerCycle = 0
  var forneyEEXlInvShiftLatency = 0
  do {
    forneyEEXlInvTermsPerCycle += 1
    forneyEEXlInvShiftLatency = math.ceil(T_LEN/forneyEEXlInvTermsPerCycle.toDouble).toInt
  } while(MSG_DURATION_CORE <= forneyEEXlInvShiftLatency)
