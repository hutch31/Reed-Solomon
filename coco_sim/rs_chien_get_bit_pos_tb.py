...
async def poly_cover(dut):
    ...
    for i in range(1, T_LEN):
        ...
        corrupts = i
        test = RsChienGetBitPos(dut, corrupts)
        test.set_if()
        test.build_env()
        test.gen_stimilus()
        await test.run()
...
def rs_chien_bit_pos_tb():

    test_module = "rs_chien_get_bit_pos_tb"
    hdl_toplevel = "RsChienGetBitPos"
    ...
    verilog_sources.append(proj_path / "RsChienGetBitPos.sv")
